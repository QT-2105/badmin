    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-5 md:px-6">
      ...
      <section className="space-y-2">
        ...
        {visibleTransactions.map((item) => (
          <article key={item.id} className="grid gap-2 rounded-xl border border-white/10 bg-white/[0.04] p-3 text-sm md:grid-cols-[120px_1fr_130px_130px] md:items-center">
            <div className={item.transactionType === 'INCOME' ? 'font-semibold text-emerald-300' : 'font-semibold text-rose-300'}>{item.transactionType === 'INCOME' ? 'Thu' : 'Chi'}</div>
            <div>
              <div className="text-white">{item.title || item.category}</div>
              <div className="text-xs text-slate-400">{getCategoryLabel(item.category)} · {item.createdAt ? new Date(item.createdAt).toLocaleString('vi-VN') : ''}</div>
            </div>
        ...
        {!isLoading && visibleTransactions.length === 0 ? (
          <div className="rounded-xl border border-dashed border-white/10 p-4 text-sm text-slate-400">Chưa có giao dịch trong kỳ báo cáo đang chọn.</div>
        ) : null}
      </section>
    </div>
  );
