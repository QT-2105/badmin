...
export function PlayerTeam({ team, teamLabel }: PlayerTeamProps) {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-1.5">
      <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-300">{teamLabel}</div>
      <div className="space-y-1">
        {team.map((player, idx) => (
          <button
            type="button"
            key={idx}
            className="grid min-h-11 w-full grid-cols-[auto_minmax(0,1fr)] items-start gap-2 rounded-lg border border-transparent p-1.5 text-left transition-colors hover:border-cyan-300/20 hover:bg-white/[0.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70"
          ...
          >
            ...
            <div className="min-w-0">
              <p className="overflow-hidden break-words text-[13px] font-bold leading-4 text-slate-50 [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2]" title={player?.name}>{getDisplayPlayerName(player?.name)}</p>
