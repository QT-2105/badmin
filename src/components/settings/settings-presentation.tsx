  ...
  return (
    <PageShell maxWidth="max-w-[1920px]">
      ...
      <SettingsCard
        ...
        title="Giao diện"
        description="Điều chỉnh chế độ sáng hoặc tối đang có của ứng dụng."
        ...
        {SETTINGS_NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={[
                'min-h-[3.75rem] w-[min(72vw,12rem)] shrink-0 rounded-lg border px-2.5 py-2 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background lg:w-auto',
                isActive
                  ? isDanger
            ...
            >
              <span className="flex items-center justify-between gap-2">
                <span className="block min-w-0 truncate text-[0.62rem] font-semibold uppercase tracking-[0.12em] opacity-75">{item.group}</span>
                <CapabilityStatusChip status={item.status} />
              </span>
              <span className="mt-1 block truncate text-sm font-semibold leading-tight">{item.label}</span>
      ...
      <div className="rounded-xl border border-border bg-surface-muted p-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-info">Xem trước</p>
            <p className="mt-1 truncate text-base font-semibold text-foreground">{branding?.clubName || clubName || 'Badmin'}</p>
            <p className="mt-0.5 text-xs text-muted-foreground">{branding?.logoUrl ? 'Logo riêng' : 'Avatar chữ'}</p>
          </div>
        </div>
        <p className="mt-3 border-t border-border pt-3 text-xs leading-5 text-muted-foreground">
          Hiển thị tại thanh menu, màn đăng nhập và khu vực nhận diện CLB.
        </p>
      </div>
      <div className="rounded-xl border border-border bg-surface-muted p-3 sm:p-4">
        <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-end">
          <label className="block min-w-0">
            ...
            />
            <span id="club-name-helper" className="mt-1 block text-xs leading-4 text-muted-foreground">
              Tên nhận diện dùng trong toàn bộ giao diện.
            </span>
          </label>
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            ...
            <Button type="button" onClick={onSaveBrandingName} disabled={updateNamePending} className="h-10 w-full rounded-lg sm:w-auto lg:min-w-[112px]">
        ...
        {brandingSaveMessage ? (
          ...
          <p className="mt-2 text-xs text-muted-foreground">Tên CLB được lưu thủ công; thay đổi chưa lưu chỉ nằm trong ô nhập.</p>
        )}
        <div className="mt-3 flex flex-col gap-3 border-t border-border pt-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="text-sm font-medium text-foreground">Logo CLB</p>
            <p className="mt-0.5 text-xs text-muted-foreground">Hỗ trợ PNG, JPEG và WebP theo validation hiện tại.</p>
          </div>
          <div className="grid grid-cols-1 gap-2 sm:flex sm:shrink-0 sm:flex-wrap">
            <label className="inline-flex h-10 w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-border bg-surface px-3 text-sm font-semibold text-foreground transition hover:bg-muted focus-within:outline-none focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:ring-offset-background sm:w-auto">
            ...
            {branding?.logoUrl ? (
              <Button type="button" variant="ghost" onClick={onDeleteLogo} disabled={deleteLogoPending} className="h-10 w-full rounded-lg sm:w-auto">
                Xóa logo
              </Button>
            ) : null}
          </div>
        ...
        {uploadLogoPending || deleteLogoPending ? (
          <p className="mt-2 text-xs text-muted-foreground">Đang cập nhật logo...</p>
        ) : null}
      </div>
    </div>
  );
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-info/25 bg-info-soft px-2 py-0.5 text-[10px] font-semibold text-info">Sáng / Tối</span>
          </div>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            Thay đổi có hiệu lực ngay, được lưu trong trình duyệt và giữ nguyên khi tải lại trang.
          </p>
        </div>
        <ThemeToggle className="h-10 shrink-0 rounded-lg" />
      </div>
    </div>
  );
}
    ...
    <div className="grid items-start gap-3 lg:grid-cols-[200px_minmax(0,1fr)]">
      ...
      <label className="rounded-xl border border-border bg-surface-muted p-3 sm:p-4">
        ...
        <Input
          ...
          className={`${formInputClass} mt-2 max-w-[120px]`}
...
function DestructiveActionSection({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-danger/25 bg-danger-soft p-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
...
function SettingsCard({
  ...
  return (
    <section
      ...
      className={cn(
        'scroll-mt-4 rounded-xl border bg-surface p-3 shadow-subtle sm:p-4',
        danger ? 'border-danger/30' : 'border-border'
      )}
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg ${danger ? 'bg-danger-soft text-danger' : 'bg-info-soft text-info'}`}>
            {icon}
          </div>
          <div className="min-w-0">
            <h2 id={titleId} className="text-base font-semibold leading-snug text-foreground sm:text-lg">{title}</h2>
      ...
      {expanded ? <div id={contentId} className="mt-3 border-t border-border pt-3">{children}</div> : null}
    </section>
  );
}
...
function SettingToggle({ checked, title, description, onChange }: { checked: boolean; title: string; description: string; onChange: (checked: boolean) => void }) {
  return (
    <label
      className={cn(
        'flex min-h-[92px] cursor-pointer flex-col gap-3 rounded-xl border p-3 transition-colors focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:ring-offset-background sm:flex-row sm:items-center sm:justify-between sm:p-4 motion-reduce:transition-none',
        checked
          ? 'border-success/30 bg-success-soft/45 hover:bg-success-soft/65'
          : 'border-border bg-surface-muted hover:border-info/25 hover:bg-muted'
      )}
    >
      ...
      <span
        className={cn(
          'flex h-10 shrink-0 self-start items-center gap-2 rounded-lg border px-2.5 sm:self-auto',
          checked ? 'border-success/25 bg-surface text-success-foreground' : 'border-border bg-background text-muted-foreground'
        )}
      >
        <span className="min-w-[4.25rem] text-right text-xs font-semibold">
