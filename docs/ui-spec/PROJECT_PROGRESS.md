...
- Sprint 11.12 did not add Runtime-specific animation and did not change countdowns, match timers, refresh intervals, retry intervals, the then-current runtime cooldown, or React Query retry configuration. The cooldown was superseded by owner-approved immediate post-match return semantics on 2026-08-13.
...
- Sprint 12.7C refined Runtime court card readability, player team typography, gender differentiation, empty-court presentation, next-match card touch targets, score tone and queue action visual hierarchy while preserving the runtime behavior that existed at that sprint. Its `JUST_FINISHED` behavior was superseded by owner-approved immediate post-match return semantics on 2026-08-13.
